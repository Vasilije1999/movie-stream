...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 20, 2022 at 10:39 PM
...
INSERT INTO `categories_m` (`id`, `c_name`) VALUES
(1, 'Action & Adventure '),
(2, 'Anime '),
(3, 'Children & Family'),
(4, 'Classic '),
...
--
-- Stand-in structure for view `vwmovies`
-- (See below for the actual view)
--
CREATE TABLE `vwmovies` (
`id` int(30)
,`title` varchar(150)
,`description` text
,`category` int(3)
...
--
-- Structure for view `vwmovies`
--
DROP TABLE IF EXISTS `vwmovies`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwmovies`  AS SELECT `movies`.`id` AS `id`, `movies`.`title` AS `title`, `movies`.`description` AS `description`, `movies`.`category` AS `category`, `movies`.`deleted` AS `deleted`, `movies`.`changed` AS `changed`, `categories_m`.`c_name` AS `c_name` FROM (`movies` join `categories_m` on(`movies`.`category` = `categories_m`.`id`)) ;
